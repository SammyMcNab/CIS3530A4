...
* Set a password by first moving into the psql terminal with the command `psql`, then doing `ALTER USER postgres PASSWORD 'yourpassword';`, replacing 'yourpassword' with your preferred password. Run `exit` to leave the query terminal, then again to exit the psql terminal.
